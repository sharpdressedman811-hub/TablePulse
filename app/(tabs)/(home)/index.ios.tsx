...
export default function HomeScreen() {
  ...
  const fetchData = useCallback(async () => {
    if (!user) return;
    console.log('[HomeScreen iOS] Fetching data for user:', user.id);
    setError('');
    ...
    try {
      ...
      if (restResult.error) {
        console.warn('[HomeScreen iOS] Restaurants fetch error:', restResult.error.message);
      } else {
        console.log('[HomeScreen iOS] Restaurants loaded:', restResult.data?.length ?? 0);
        setRestaurants(restResult.data ?? []);
      }

      if (recResult.error) {
        console.warn('[HomeScreen iOS] Recommendations fetch error:', recResult.error.message);
      } else {
        console.log('[HomeScreen iOS] Recommendations loaded:', recResult.data?.length ?? 0);
        setRecommendations(recResult.data ?? []);
      }
    } catch (err: any) {
      console.error('[HomeScreen iOS] Fetch error:', err);
  ...
  const handleRefresh = () => {
    console.log('[HomeScreen iOS] Pull-to-refresh triggered');
    setRefreshing(true);
    fetchData();
  };
    ...
    <>
      ...
      >
            ...
            <View style={styles.recList}>
              {displayRecs.map((rec) => (
                <RecommendationCard
                  ...
                  compact
                  onPress={() => {
                    console.log('[HomeScreen iOS] Recommendation tapped:', rec.id);
        ...
        {isFounder && (
          <AnimatedPressable
            onPress={() => {
              console.log('[HomeScreen iOS] Founder View link tapped — navigating to founder-dashboard');
